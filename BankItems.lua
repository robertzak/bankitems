...
local GetCurrencyListInfo, GetCurrencyListLink, GetCurrencyListSize = GetCurrencyListInfo, GetCurrencyListLink, GetCurrencyListSize
...
local GetMerchantItemCostItem, GetBackpackCurrencyInfo, GetAuctionItemLink = GetMerchantItemCostItem, GetBackpackCurrencyInfo, GetAuctionItemLink
local GetRecipeItemLink, GetRecipeReagentItemLink, GetQuestLogItemLink, GetQuestItemLink = C_TradeSkillUI.GetRecipeItemLink, C_TradeSkillUI.GetRecipeReagentItemLink, GetQuestLogItemLink, GetQuestItemLink
...
function BankItems_Bag_OnClick(self, button)
	...
	elseif bagID == 104 and not BankItems_Save.voidBags then --use new Void Storage Frame
		...
		if ( bagID == 0 ) then
			...
			frame:SetHeight(BACKPACK_BASE_HEIGHT + middleBgHeight);
		else
			if (size == 1) then
				-- Halloween gag gift
				bgTexture1Slot:Show();
...
function BankItems_Frame_OnEvent(self, event, ...)
	...
	elseif event == "VOID_STORAGE_UPDATE" or event == "VOID_STORAGE_CONTENTS_UPDATE" or event == "VOID_TRANSFER_DONE" or event == "VOID_STORAGE_OPEN" then
...
function BankItems_GBFrame_OnShow()
	...
	if not BankItems_GuildDropdown.selectedValue then
		...
		selfGuildRealm = selfGuildRealm and strtrim(selfGuildRealm) or strtrim(GetRealmName())
...
function GetCurrencyString(name1)
	...
	for i=1, GetCurrencyListSize() do
		local name2 = GetCurrencyListInfo(i)
		if name1 == name2 then
		...
		if snaptime then
			...
			for i = 1, #selfPlayer.Bag103 do
				if selfPlayer.Bag103[i-removed] then
					...
					if e then
						if (e == 1 and elapsed > 30*60 + 3600) or (e == 2 and elapsed > 2*60*60 + 3600) or (e == 3 and elapsed > 12*60*60 + 3600) or (e == 4 and elapsed > 48*60*60 + 3600) then
...
function BankItems_SaveReagentBank()
	...
	for slot = 1, NUM_REAGENTBANKGENERIC_SLOTS do
		texture, count, _, _, _, _, link = GetContainerItemInfo(REAGENTBANK_CONTAINER, slot)
...
function BankItems_SaveAuctions()
	local name, texture, count, saleStatus, itemPointer
	...
	for i = 1, totalAuctions do
		name, texture, count, _, _, _, _, _, _, _, _, _, _, _, _, saleStatus = GetAuctionItemInfo("owner", i)
...
function UpdateClosedBankCounts(slot) --update counts that may have changed from crafting and quest turn-ins which can now be taken directly from the bank
	...
	for bagNum = 5, 11 do --count equipped bank bag items
	...
	for bagNum = 5, 11 do --adjust equipped bank bag items
		theBag = selfPlayer[format("Bag%d", bagNum)]
		if theBag and theBag.size then
			for bagItem = 1, theBag.size do
				if theBag[bagItem] and theBag[bagItem].link and type(theBag[bagItem].link) == "string" then
...
function BankItems_UserDropdownGenerateKeys(sortTable, saveTable, showAll)
	...
	for key, value in pairs(saveTable) do
		if type(value) == "table" and key ~= "Behavior" and key ~= "Behavior2" then
			local _, realm = strsplit("|", key)
			if showAll or BankItems_Save.ShowAllRealms or (realm == selfPlayerRealm and (BankItems_Save.ShowOppositeFaction or value.faction == selfPlayer.faction)) then
...
function BankItems_GenerateExportText()
	...
	if BankItems_Save.GroupExportData then
		...
		local data = newTable()
		local _, itemName, itemType
		for num = 1, NUM_BANKGENERIC_SLOTS do
			if bankPlayer[num] then
				itemName, _, _, _, _, itemType = GetItemInfo(bankPlayer[num].link)
			...
			--if theBag then
				...
				for bagItem = 1, realSize or 0 do -- if realSize is nil skip the loop since table is empty or has an error
					if theBag[bagItem] and type(theBag[bagItem].link) == "string" then
						itemName, _, _, _, _, itemType = GetItemInfo(theBag[bagItem].link)
				...
				if BankItems_Save.Behavior2[1] then
					for num = 1, NUM_BANKGENERIC_SLOTS do
						if bankPlayer[num] then
							...
							templocalized = BankItems_SpecialCharactersLocalization(strmatch(bankPlayer[num].link, "%[(.*)%]"))
					...
					if searchFilter[bagNum] and theBag then
						...
						for bagItem = 1, realSize or 0 do -- if realSize is nil skip the loop since table is empty or has an error
							if theBag[bagItem] and type(theBag[bagItem].link) == "string" then
								...
								templocalized = BankItems_SpecialCharactersLocalization(strmatch(theBag[bagItem].link, "%[(.*)%]"))
								if strfind(templocalized, searchText, 1, true) then
									data[temp] = data[temp] or newTable()
									data[temp][key] = data[temp][key] or newTable()
						...
						if type(theBag.link) == "string" then
							...
							templocalized = BankItems_SpecialCharactersLocalization(strmatch(theBag.link, "%[(.*)%]"))
							if strfind(templocalized, searchText, 1, true) then
								data[temp] = data[temp] or newTable()
								data[temp][key] = data[temp][key] or newTable()
				...
				if BankItems_Save.Behavior2[1] then
					for num = 1, NUM_BANKGENERIC_SLOTS do
						if bankPlayer[num] then
							...
							temp = BankItems_SpecialCharactersLocalization(strmatch(bankPlayer[num].link, "%[(.*)%]"))
					...
					if searchFilter[bagNum] and theBag then
						...
						for bagItem = 1, realSize or 0 do -- if realSize is nil skip the loop since table is empty or has an error
							if theBag[bagItem] and type(theBag[bagItem].link) == "string" then
								...
								temp = BankItems_SpecialCharactersLocalization(strmatch(theBag[bagItem].link, "%[(.*)%]"))
								if strfind(temp, searchText, 1, true) then
									count = count + 1
									if count == 1 then
										line = line + 1
						...
						if type(theBag.link) == "string" then
							temp = BankItems_SpecialCharactersLocalization(strmatch(theBag.link, "%[(.*)%]"))
							if strfind(temp, searchText, 1, true) then
								count = count + 1
								if count == 1 then
									line = line + 1
...
function BankItems_Hooktooltip(tooltip)
	...
	tooltip.SetAuctionItem = function(self, ...)
		self.BankItems_Link = GetAuctionItemLink(...)
		return p(self, ...)
	end
...
do
	...
	BankItems_OptionsFrame_TooltipInfo:SetScript("OnClick", function(self)
		if BankItems_Save.TooltipInfo then
			...
			BlizzardOptionsPanel_CheckButton_Enable(BankItems_OptionsFrame_TTUnique, true)
	...
	BankItems_OptionsFrame_TTUnique:SetPoint("TOPLEFT", 16, -189)
	BankItems_OptionsFrame_TTUnique:SetHitRectInsets(0, -300, 0, 0)
	...
	BankItems_DelUserDropdown:SetPoint("TOPLEFT", 5, -477)
	...
	BankItems_DelUserDropdownLabel:SetText(L["Delete a Character:"]) --new text string to localize
	
	BankItems_DelGuildDropdown = CreateFrame("Frame", "BankItems_DelGuildDropdown", BankItems_OptionsFrame, "UIDropDownMenuTemplate")
	BankItems_DelGuildDropdown:SetPoint("TOPLEFT", 5, -522)
	...
	BankItems_DelGuildDropdownLabel:SetText(L["Delete a Guild:"]) --new text string to localize
